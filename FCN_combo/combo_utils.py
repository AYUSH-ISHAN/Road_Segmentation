...
def Batch_Generator_Function(data_folder, image_shape):
...
def output_generator(sess, logits, keep_prob, image_pl, data_folder, image_shape):
        ...
        for file2image in glob(os.path.join(data_folder, '*.png')):
            ...
            image = scipy.misc.imresize(scipy.misc.imread(file2image), image_shape)
            tic = time.time()
            ...
            yield os.path.basename(file2image), np.array(street_im), flash
