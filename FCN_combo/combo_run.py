...
CLOUD_MODE = False   
'''
Changes the directory of everything..
Useful when you want to give a new start.
'''
...
def main_runner():
    ...
    with tf.compat.v1.Session() as sess:
        ...
        get_batches_fn = combo_utils.Batch_Generator_Function(os.path.join(data_dir, 'data_road/training'), image_shape)

        epochs = 50
        batch_size = 8 #5 the batch size '5' also worked great.
...
def guessing_or_predicting(test_data_path, print_speed=False):
    ...
    with tf.compat.v1.Session() as sess:
        ...
        # Predict the samples
        combo_utils.predictometer(runs_dir, test_data_path, sess, image_shape, logits, keep_prob, input_image, print_speed)
