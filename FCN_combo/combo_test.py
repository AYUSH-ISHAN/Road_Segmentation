...
def test_video(sess, image_shape, logits, keep_prob, input_image):
    if SAVING:
        ...
        video_writer = cv2.VideoWriter('./video_predictions/Video_Of_Travel.avi', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 30,
                                       (576, 160))
    else:
        cap = cv2.VideoCapture('./Video Of Travel.mp4')
...
'''
Adjust the below argument if enabled the CLOUD_MODE while training.
'''
