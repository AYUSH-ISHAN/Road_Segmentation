    ...
    def get_batches_fn(batch_size):
        ...
        for batch_i in range(0, len(image_paths), batch_size):
            ...
            for image_file in image_paths[batch_i:batch_i+batch_size]:
                ...
                #numpy.array(Image.fromarray(image_file).resize(image_shape))#scipy.misc.imresize(scipy.misc.imread(image_file), image_shape)
                gt_image = cv2.resize(cv2.imread(gt_image_file), image_shape)
                #cipy.misc.imresize(scipy.misc.imread(gt_image_file), image_shape)
                #numpy.array(Image.fromarray(gt_image_file).resize(image_shape))#scipy.misc.imresize(scipy.misc.imread(gt_image_file), image_shape)
...
def gen_test_output(sess, logits, keep_prob, image_pl, data_folder, image_shape):
    ...
    for image_file in glob(os.path.join(data_folder, 'image_2', '*.png')):
        ...
        street_im = Image.fromarray((image * 255).astype(np.uint8))
